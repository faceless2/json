 ...
 * used for encryption. In practice it may contain two keys (public AND private).
 * @since 5
 * @see JWT
 * @see COSE
 */
    ...
    private static List<Key> getKeys(final Json j, Provider provider) {
        ...
        try {
            ...
            if ("EC".equals(kty)) {
                ...
                String crv = j.stringValue("crv");
                java.security.spec.NamedParameterSpec namedParamSpec;
                if ("Ed25519".equals(crv)) {
                    namedParamSpec = java.security.spec.NamedParameterSpec.ED25519;
                } else if ("Ed448".equals(crv)) {
                    namedParamSpec = java.security.spec.NamedParameterSpec.ED448;
                } else if ("X25519".equals(crv)) {
                    namedParamSpec = java.security.spec.NamedParameterSpec.X25519;
                } else if ("X448".equals(crv)) {
                    namedParamSpec = java.security.spec.NamedParameterSpec.X448;
                } else {
                ...
                if (j.has("x")) {
                    ...
                    EdECPublicKeySpec spec = new EdECPublicKeySpec(namedParamSpec, new EdECPoint(xIsOdd, y));
                    keys.add(factory.generatePublic(spec));
                }
                if (j.has("d")) {
                    byte[] d =  JWT.base64decode(j.stringValue("d"));
                    EdECPrivateKeySpec spec = new EdECPrivateKeySpec(namedParamSpec, d);
                    keys.add(factory.generatePrivate(spec));
                }
            } else {
    ...
    public void setKeys(Collection<Key> keys) {
        ...
        for (Key key : keys) {
            ...
            if (key instanceof ECKey) {
                ...
                put("kty", "OKP");
                alg = "EdDSA";
                NamedParameterSpec np = ((EdECKey)key).getParams();
                if (np.getName().equals(NamedParameterSpec.ED25519.getName())) {
                    put("crv", "Ed25519");
                } else if (np.getName().equals(NamedParameterSpec.ED448.getName())) {
                    put("crv", "Ed448");
                } else if (np.getName().equals(NamedParameterSpec.X25519.getName())) {
                    put("crv", "X25519");
                } else if (np.getName().equals(NamedParameterSpec.X448.getName())) {
                    put("crv", "X448");
                } else {
                    throw new IllegalArgumentException("Unknown curve \"" + np.getName() + "\"");
                }
                ...
                if (key instanceof EdECPublicKey) {
                    ...
                    seenpub = true;
                    EdECPublicKey k = (EdECPublicKey)key;
                    boolean xodd = k.getPoint().isXOdd();
                    BigInteger y = k.getPoint().getY();
                    ...
                    seenpri = true;
                    EdECPrivateKey k = (EdECPrivateKey)key;
                    byte[] d = k.getBytes().get();
                    put("d", d);
                }
            } else {
        ...
        if ("RS256".equals(alg) || "RS384".equals(alg) || "RS512".equals(alg)) {
            ...
            if (key instanceof EdECKey) {
                ...
                if (np.getName().equals(NamedParameterSpec.ED25519.getName())) {
                    ...
                    throw new NoSuchAlgorithmException("No JWT algorithm matches \"" + np.getName() + "\" EdDSA private key"); 
                }
            }
        } else {
